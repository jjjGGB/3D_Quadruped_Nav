...
struct NodeConfig
{
    std::string imu_topic = "/LIDAR/IMU";
    std::string lidar_topic = "/LIDAR/POINTS";
    std::string body_frame = "body";
    std::string world_frame = "lidar";
    bool publish_tf = true;
    bool pub_world_cloud = false;
    bool print_time_cost = false;
    double imu_acc_scale = 1.0;
    double body_cloud_voxel_size = 0.1;
    int esti_il_print_every = 20;
};
struct StateData
{
...
public:
    LIONode() : Node("lio_node")
    {
        ...
        if (m_node_config.pub_world_cloud)
            m_world_cloud_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("world_cloud", 2);
        m_path_pub = this->create_publisher<nav_msgs::msg::Path>("lio_path", 2);
        m_odom_pub = this->create_publisher<nav_msgs::msg::Odometry>("lio_odom", 2);
    ...
    void loadParameters()
    {
        ...
        if (config["pub_world_cloud"])
            m_node_config.pub_world_cloud = config["pub_world_cloud"].as<bool>();
        ...
        if (config["esti_il_print_every"])
            m_node_config.esti_il_print_every = std::max(1, config["esti_il_print_every"].as<int>());
        ...
        if (config["body_cloud_voxel_size"])
            m_node_config.body_cloud_voxel_size = std::max(0.0, config["body_cloud_voxel_size"].as<double>());

        if (m_node_config.body_cloud_voxel_size > 0.0)
        {
            const float leaf_size = static_cast<float>(m_node_config.body_cloud_voxel_size);
            m_body_cloud_filter.setLeafSize(leaf_size, leaf_size, leaf_size);
        }
    }
    ...
    void imuCB(const sensor_msgs::msg::Imu::SharedPtr msg)
    {
        ...
        m_state_data.imu_buffer.emplace_back(V3D(msg->linear_acceleration.x, msg->linear_acceleration.y, msg->linear_acceleration.z) * m_node_config.imu_acc_scale,
    ...
    {
        CloudType::Ptr cloud = Utils::cloud2PCL(msg, m_builder_config.lidar_filter_num, m_builder_config.lidar_min_range, m_builder_config.lidar_max_range);
    ...
    void timerCB()
    {
        ...
        if (m_builder_config.esti_il)
        {
            ++m_esti_il_step_count;
            if (m_esti_il_step_count % m_node_config.esti_il_print_every == 0)
            {
                const auto &r_il = m_kf->x().r_il;
                const auto &t_il = m_kf->x().t_il;
                std::ostringstream oss;
                oss << std::setprecision(4) << std::defaultfloat;
                oss << "Estimated extrinsic (step " << m_esti_il_step_count << "):\n";
                oss << "t_il = [" << t_il.x() << ", " << t_il.y() << ", " << t_il.z() << "]\n";
                oss << "r_il =\n";
                oss << "[" << r_il(0, 0) << ", " << r_il(0, 1) << ", " << r_il(0, 2) << "]\n";
                oss << "[" << r_il(1, 0) << ", " << r_il(1, 1) << ", " << r_il(1, 2) << "]\n";
                oss << "[" << r_il(2, 0) << ", " << r_il(2, 1) << ", " << r_il(2, 2) << "]";
                RCLCPP_INFO(this->get_logger(), "%s", oss.str().c_str());
            }
        }

        // FASTLIO2 发布局部里程计 TF：world_frame -> body_frame。
        // 在重定位链路中推荐配置为 lio_odom -> base_link，localizer 再发布 map -> lio_odom。
        // 不要让 Gazebo 的 world_tf_publisher 同时发布 base_link，否则 RViz 会在两套动态 TF 间跳变。
        ...
        CloudType::Ptr body_cloud_pub = body_cloud;
        if (m_node_config.body_cloud_voxel_size > 0.0)
        {
            body_cloud_pub.reset(new CloudType);
            m_body_cloud_filter.setInputCloud(body_cloud);
            m_body_cloud_filter.filter(*body_cloud_pub);
        }

        publishCloud(m_body_cloud_pub, body_cloud_pub, m_node_config.body_frame, m_package.cloud_end_time);
...
private:
    ...
    uint64_t m_esti_il_step_count = 0;
    std::shared_ptr<IESKF> m_kf;
    std::shared_ptr<MapBuilder> m_builder;
    std::shared_ptr<tf2_ros::TransformBroadcaster> m_tf_broadcaster;
    pcl::VoxelGrid<PointType> m_body_cloud_filter;
};
