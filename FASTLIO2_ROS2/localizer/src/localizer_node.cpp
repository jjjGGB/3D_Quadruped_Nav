...
public:
    LocalizerNode() : Node("localizer_node")
    {
        ...
        m_confidence_pub = this->create_publisher<std_msgs::msg::Float32>("confidence", 10);
    ...
    void timerCB()
    {
        if (!m_state.message_received)
        {
            RCLCPP_WARN_THROTTLE(
                ...
                "Waiting for synchronized inputs: cloud_topic=%s and odom_topic=%s. "
                "The relocalize service only loads the PCD map and initial guess; map->local TF is published after ICP receives cloud+odom.",
                m_config.cloud_topic.c_str(), m_config.odom_topic.c_str());
            return;
        }
        ...
        M4F initial_guess = M4F::Identity();
        bool use_service_guess = false;
        {
            std::lock_guard<std::mutex> lock(m_state.service_mutex);
            use_service_guess = m_state.service_received;
            if (use_service_guess)
        ...
        if (result)
        {
            ...
            if (!m_state.first_tf_sent)
            {
                RCLCPP_INFO(
                    this->get_logger(),
                    "ICP localized successfully. Publishing TF %s -> %s.",
                    m_config.map_frame.c_str(), m_config.local_frame.c_str());
                m_state.first_tf_sent = true;
            }
        }
        else
        {
            RCLCPP_WARN_THROTTLE(
                ...
                "ICP has not converged yet. rough_score=%.4f/%.4f refine_score=%.4f/%.4f. "
                "Check initial pose, PCD/map frame, and whether /fastlio2/body_cloud overlaps the map.",
                m_localizer->roughScore(), m_localizer_config.rough_score_thresh,
                m_localizer->score(), m_localizer_config.refine_score_thresh);
        }
        publishConfidence(result);
    ...
    void syncCB(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud_msg, const nav_msgs::msg::Odometry::ConstSharedPtr &odom_msg)
    {
        ...
        if (!m_state.message_received)
        {
            ...
            RCLCPP_INFO(
                this->get_logger(),
                "Received first synchronized cloud+odom. cloud_frame=%s odom_frame=%s child_frame=%s cloud_points=%zu",
                cloud_msg->header.frame_id.c_str(),
                odom_msg->header.frame_id.c_str(),
                odom_msg->child_frame_id.c_str(),
                m_state.last_cloud->size());
        }
    }
    ...
    void relocCB(const std::shared_ptr<interface::srv::Relocalize::Request> request, std::shared_ptr<interface::srv::Relocalize::Response> response)
    {
        ...
        }
        {
            ...
            m_state.first_tf_sent = false;
        }

        response->success = true;
        response->message = "relocalize request accepted; wait for /localizer/relocalize_check or TF map->local frame to confirm ICP convergence";
        return;
    }
    ...
    // 发布定位置信度，范围 [0.0, 1.0]，越接近 1 表示配准越可靠
    // 由 refine ICP 的 fitness score（平均配准误差）映射得到：
    //   score = 0           -> confidence = 1.0
    //   score >= 阈值        -> confidence = 0.0
    // 本帧配准失败时置信度为 0.0
    void publishConfidence(bool result)
    {
        std_msgs::msg::Float32 msg;
        double thresh = m_localizer_config.refine_score_thresh;
        if (!result || thresh <= 0.0)
        {
            msg.data = 0.0f;
        }
        else
        {
            double conf = 1.0 - m_localizer->score() / thresh;
            conf = std::max(0.0, std::min(1.0, conf));
            msg.data = static_cast<float>(conf);
        }
        m_confidence_pub->publish(msg);
    }
...
private:
    ...
    rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr m_confidence_pub;
};
