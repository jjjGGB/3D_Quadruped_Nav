...
bool ICPLocalizer::align(M4F &guess)
{
    ...
    m_rough_score = std::numeric_limits<double>::max();
    if (m_refine_tgt->size() == 0 || m_rough_tgt->size() == 0 ||
        m_refine_inp->size() == 0 || m_rough_inp->size() == 0)
        return false;
    ...
    m_rough_score = m_rough_icp.getFitnessScore();
    if (!m_rough_icp.hasConverged() || m_rough_score > m_config.rough_score_thresh)
        return false;
    ...
    m_score = m_refine_icp.getFitnessScore();
    if (!m_refine_icp.hasConverged() || m_score > m_config.refine_score_thresh)
        return false;
