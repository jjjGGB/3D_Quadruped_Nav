...
{
    // robot_path 由 FSM 传入，例如 go2/himloco 或 go2/robot_lab。
    // ReadYamlRL() 先读取 policy/<robot_path>/config.yaml，其中 model_name 指定 .pt 文件名。
    ...
    // 最终权重路径在这里拼接：
    //   <CMAKE_CURRENT_SOURCE_DIR>/policy/<robot_path>/<model_name>
    // 例如当前 go2 FSM 默认 robot_path=go2/himloco，config.yaml 中 model_name=himloco.pt，
    // 所以实际加载 src/rl_sar/rl_sar/policy/go2/himloco/himloco.pt。
    // 这里使用 torch::jit::load，要求 .pt 是 TorchScript 导出的模型，而不是普通 PyTorch checkpoint。
