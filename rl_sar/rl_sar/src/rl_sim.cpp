...
RL_Sim::RL_Sim() : rclcpp::Node("rl_sim_node")
{
    ...
    // 1. 从 param_node 读取本次仿真的机器人名称。
    // robot_name 决定后续读取 policy/<robot_name>/base.yaml，例如 go2 -> policy/go2/base.yaml。
    // gazebo_model_name 目前主要用于和 Gazebo 中生成的模型名保持一致。
    ...
    // 2. 读取基础机器人参数：关节名、关节映射、PD 参数、默认站立角度、控制周期等。
    // 注意：base.yaml 不包含 .pt 权重路径，它只描述机器人低层控制参数。
    this->ReadYamlBase(this->robot_name);

    // 3. 根据 robot_name 创建对应 FSM。
    // 对 go2 来说会注册 policy/go2/fsm.hpp 中的状态机：
    // Passive -> GetUp -> RL_Locomotion -> GetDown。
    // 真正加载 RL 策略权重的动作发生在 FSM 进入 RLFSMStateRL_Locomotion 时。
    ...
    // 4. Torch 推理运行在部署模式：关闭 autograd，只做前向推理。
    torch::autograd::GradMode::set_enabled(false);
    torch::set_num_threads(4);

    // 5. 初始化机器人命令/状态缓存。
    // robot_command_publisher_msg 最终发布到 robot_joint_controller/command；
    // robot_state_subscriber_msg 来自 robot_joint_controller/state。
    ...
    // 6. 启动 ros2_control 中的 robot_joint_controller，并发布关节命令。
    // RL 输出的目标关节位置/速度/力矩会经 SetCommand() 写入该 topic。
    ...
    // 7. 订阅部署所需输入。
    // /cmd_vel：导航模式下的速度指令，来自 web/d1_controller/Nav2 等上层导航。
    // /joy：手柄输入，可切换状态和手动给速度。
    // /imu：Gazebo IMU，用于构造策略观测中的角速度和姿态。
    // robot_joint_controller/state：关节位置、速度、估计力矩。
    ...
    // 9. 两个核心循环。
    // loop_control: dt 周期，执行 FSM/PD 控制并发布关节命令。
    // loop_rl: dt * decimation 周期，构造观测并调用 TorchScript 模型前向推理。
    // 例如 go2 base.yaml: dt=0.005, decimation=4，则控制 200Hz，RL 推理 50Hz。
    ...
    // 10. 键盘循环用于调试和手动切状态：0 起身准备，1 进入 RL 行走，9 趴下，N 切导航模式。
    this->loop_keyboard = std::make_shared<LoopFunc>("loop_keyboard", 0.05, std::bind(&RL_Sim::KeyboardInterface, this));
    this->loop_keyboard->start();

    // 11. Action Server 给外部节点一个程序化接口：
    // robot_activation 等价于按 0/1 或 9；
    // set_locomotion_mode 等价于按数字键；
    // set_navigation_mode 等价于按 N。
...
void RL_Sim::StartJointController(const std::string &ros_namespace, const std::vector<std::string> &names)
{
    ...
    // ros2_control 的 spawner 需要知道控制哪些关节。
    // 这里临时生成 controller 参数文件，把 base.yaml 中的 joint_names 写进去。
...
{
    // 从 ROS topic 缓存转换为 RL SDK 内部状态：
    // IMU 四元数/角速度 + 按 joint_mapping 重排后的关节 q/dq/tau_est。
...
{
    // 将 FSM/PD 生成的内部关节命令按 joint_mapping 写回 ROS 消息。
    // 这一步完成后 robot_joint_controller 会在 Gazebo 中驱动各关节。
    for (int i = 0; i < this->params.num_of_dofs; ++i)
    {
...
{
    // 高频控制循环，频率为 params.dt。
    // 它不直接运行神经网络，而是消费 RunModel() 放入队列中的 RL 输出，
    // 再通过 FSM 状态决定当前是被动、起身、RL 行走还是趴下。
    ...
    if (simulation_running)
    {
        ...
        {
            // navigation_mode=true 时，RunModel() 会使用 /cmd_vel 作为策略命令；
            // false 时使用键盘/手柄写入的 control.x/control.y/control.yaw。
...
{
    // 导航速度入口。web 导航或路径跟踪节点最终应发布 geometry_msgs/Twist 到 /cmd_vel。
    // 只有 control.navigation_mode=true 时，RunModel() 才会把这里的 cmd_vel 喂给 RL 策略。
    this->cmd_vel = *msg;
}
...
{
    // 低频 RL 推理循环，频率为 params.dt * params.decimation。
    // 前提：
    // 1) rl_init_done=true：FSM 已进入 RL_Locomotion 并完成 InitRL()；
    // 2) simulation_running=true：仿真没有被暂停。
    if (this->rl_init_done && simulation_running)
    {
        ...
        {
            // 导航模式：上层规划/跟踪发布 /cmd_vel，RL 策略只负责把速度命令变成关节动作。
            this->obs.commands = torch::tensor({{this->cmd_vel.linear.x, this->cmd_vel.linear.y, this->cmd_vel.angular.z}});
        }
        else
        {
            // 手动模式：键盘 WASD/QE 或手柄摇杆直接写 control.x/y/yaw。
        ...
        // Forward() 调用已经加载好的 TorchScript 模型，输出 12 维 action。
...
torch::Tensor RL_Sim::Forward()
{
    ...
    // 根据 config.yaml 中 observations 的顺序拼接观测并裁剪。
    // go2/himloco 默认观测包含 commands、ang_vel、gravity_vec、dof_pos、dof_vel、actions，
    // 且 observations_history=[0..5]，所以会把最近 6 帧观测拼成历史输入。
    ...
    if (this->params.observations_history.size() != 0)
    {
        ...
        // this->model 是 RL::InitRL() 中通过 torch::jit::load(...) 加载的 .pt TorchScript 权重。
